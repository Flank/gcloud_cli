...
class Create(base.CreateCommand):
  ...
  detailed_help = {
      ...
      'EXAMPLES':
        ...
        To update a template named "dns-restricted" to allow requestors to specify subject:
          ...
          $ {command} dns-restricted --copy-subject
  ...
  def _UpdateCertificateTemplateFromArgs(self, args, current_labels):
    ...
    if args.IsSpecified('copy_sans') or args.IsSpecified(
        'copy_subject') or args.IsSpecified('identity_cel_expression'):
      template_to_update.identityConstraints = flags_v1.ParseIdentityConstraints(
          args)
    ...
    if args.IsSpecified('predefined_values_file'):
      template_to_update.predefinedValues = flags_v1.ParsePredefinedValues(args)
    ...
    known_exts_flags = args.IsSpecified(
        'copy_known_extensions') or args.IsSpecified('drop_known_extensions')
    oid_exts_flags = args.IsSpecified(
        'copy_extensions_by_oid') or args.IsSpecified('drop_oid_extensions')
    if known_exts_flags or oid_exts_flags:
      # Parse all extension flags into a CertificateExtensionConstraints
      # message.
      template_to_update.passthroughExtensions = flags_v1.ParseExtensionConstraints(
          args)
      if known_exts_flags:
  ...
  def Args(parser):
    ...
    flags_v1.AddIdentityConstraintsFlags(
        parser, require_passthrough_flags=False)
    flags_v1.AddExtensionConstraintsFlagsForUpdate(parser)
