...
def _GetAndValidateCmekKeyName(args, is_main):
  """Parses the CMEK resource arg, makes sure the key format was correct."""
  kms_ref = args.CONCEPTS.kms_key.Parse()
  if kms_ref:
    # Since CMEK is required for replicas of CMEK mains, this prompt is only
    # actionable for main instances.
    if is_main:
      _ShowCmekPrompt()
...
class _BaseInstances(object):
  ...
  def ConstructCreateInstanceFromArgs(cls,
    ...
    is_main = instance_resource.mainInstanceName is None
    key_name = _GetAndValidateCmekKeyName(args, is_main)
    if key_name:
