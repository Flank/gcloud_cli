...
def _is_singleton_list(testcases):
...
def _parameter_decorator(naming_type, testcases):
  ...
  if _is_singleton_list(testcases):
...
def named_parameters(*testcases):
  ...
  Args:
    *testcases: Parameters for the decorated method, either a single
                iterable, or a list of tuples or dicts.
...
def _update_class_dict_for_param_test_case(
  ...
  for idx, func in enumerate(iterator):
    ...
    assert test_method_id not in six.itervalues(test_method_ids), (
