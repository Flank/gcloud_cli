...
class ImagesLabelsTestBase(sdk_test_base.WithFakeAuth,
                           cli_test_base.CliTestBase):
  """Base class for testing image labels."""
  ...
  def SetUp(self):
    ...
    self.service = self.apitools_client.images
    self.global_operations = self.apitools_client.globalOperations

    self.StartPatch('time.sleep')

  def _GetImageRef(self, name):
    params = {'project': self.Project()}

    return self.resources.Parse(
        name, params=params, collection='compute.images')
  ...
  def _MakeImageProto(self, image_ref, labels=None, fingerprint=None):
    msg = self.messages.Image()
  ...
  def _ExpectGetRequest(self, image_ref, image=None, exception=None):
    request_type = self.messages.ComputeImagesGetRequest
    self.service.Get.Expect(
        request=request_type(**image_ref.AsDict()),
        response=image,
        exception=exception)
  ...
  def _ExpectLabelsSetRequest(
    ...
    request = self.messages.ComputeImagesSetLabelsRequest(
    ...
    self.service.SetLabels.Expect(
        request=request, response=image, exception=exception)
