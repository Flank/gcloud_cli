...
"""ai-platform jobs describe tests."""
...
class DescribeTestBase(object):
  ...
  def testDescribe(self, module_name):
    ...
    self.assertEqual(
        self.Run('{} jobs describe opId'.format(module_name)), expected_job)

  def testDescribe_Epilogue(self, module_name):
    self.ExpectJob(self._GetJobSuccess())
    properties.VALUES.core.user_output_enabled.Set(True)

    self.Run('{} jobs describe opId'.format(module_name))
...
""")
  ...
  def testDescribeSummarizeTraining(self, module_name):
    ...
    properties.VALUES.core.user_output_enabled.Set(True)
    self.ExpectJob(self._GetJobSuccess())
    self.Run('{} jobs describe opId --summarize'.format(module_name))
...
+--------+----------------------+----------------------+----------------------+-----------+
  ...
  def testDescribeSummarizeHPTraining(self, module_name):
...
+--------+----------------------+----------------------+----------------------+-----------+
  ...
  def testDescribeSummarizePredict(self, module_name):
...
+--------+----------------------+----------------------+----------------------+-----------+
  ...
  def testDescribeSummarizeWithFormatFlag(self, module_name):
    """Test summarize output with --format flag displays warning."""
    properties.VALUES.core.user_output_enabled.Set(True)
    self.ExpectJob(self._GetJobSuccess(training=False))
    self.Run("""\
    {} jobs describe opId --summarize --format='yaml'
    """.format(module_name))
