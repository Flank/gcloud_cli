...
class ServicesUpdateTest(test_base.PackageUnitTestBase, parameterized.TestCase):
  ...
  def testUpdate_Succeed(self):
    svc_name = 'bar'
    command = """kuberun core services update {} --cluster foo
    ...
    self.AssertExecuteCalledOnce(command_args=[
        'core', 'services', 'update', svc_name, '--cluster', 'foo',
        '--cluster-location', 'us-central1', '--image', 'newImage'
    ])
    self._AssertContains('Service [{}] has been updated.'.format(svc_name),
                         result, 'result')
