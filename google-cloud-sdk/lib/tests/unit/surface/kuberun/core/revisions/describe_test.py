...
"""kuberun surface revisions describe tests."""
...
class RevisionsDescribeTest(test_base.PackageUnitTestBase):
  ...
  def testDescribe_Fail(self):
    command = '''kuberun core revisions describe --cluster foo
    --cluster-location us-central1 hello.example.com'''
    self.mock_bin_exec.return_value = (
        bin_ops.BinaryBackedOperation.OperationResult(
            command, status=1, errors='no revision hello.example.com found'))

    with self.assertRaises(exceptions.Error) as context:
      self.Run(command)

    self.assertIn('Cannot find revision [hello.example.com]',
