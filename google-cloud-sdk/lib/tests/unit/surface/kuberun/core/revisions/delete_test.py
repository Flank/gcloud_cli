...
"""kuberun surface revisions delete tests."""
...
class RevisionsDeleteTest(test_base.PackageUnitTestBase):
  ...
  def testDelete(self):
    ...
    command = '''kuberun core revisions delete bar --cluster foo
    ...
    self.AssertExecuteCalledOnce(command_args=[
        'core', 'revisions', 'delete', 'bar', '--cluster', 'foo',
        '--cluster-location', 'us-central1'
    ])
    self.AssertErrContains('Revision is successfully deleted.')
  ...
  def testDeleteWithError(self):
    ...
    self.mock_bin_exec.return_value = (
        bin_ops.BinaryBackedOperation.OperationResult(
            command,
            errors='revisions.serving.knative.dev "bar" not found',
            status=1))
    self.WriteInput('Y\n')
    ...
    self.AssertErrContains('revisions.serving.knative.dev "bar" not found')
