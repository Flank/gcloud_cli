...
class ComponentsAddTest(test_base.PackageUnitTestBase):

  def testAdd(self):
    command = 'kuberun components add my-component --devkit ff-go --type http'
    ...
    self.AssertExecuteCalledOnce(command_args=[
        'components', 'add', 'my-component', '--devkit', 'ff-go', '--type',
        'http'
    ])
