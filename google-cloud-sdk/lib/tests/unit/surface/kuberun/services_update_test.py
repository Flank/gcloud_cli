...
class ServicesUpdateTest(test_base.PackageUnitTestBase, parameterized.TestCase):

  def SetUp(self):
    self.mock_bin_exec = self.StartObjectPatch(
        bin_ops.StreamingBinaryBackedOperation, '_Execute')

  def testUpdate_Succeed(self):
    svc_name = 'bar'
    command = '''kuberun clusters services update --cluster foo
    ...
    self.mock_bin_exec.return_value = bin_ops.BinaryBackedOperation.OperationResult(
        command, output='Service [{}] has been updated.'.format(svc_name))

    result = self.Run(command)

    self.AssertExecuteCalledOnce(
        command_args=['clusters', 'services', 'update', '--cluster', 'foo',
                      '--cluster-location', 'us-central1', svc_name, '--image',
                      'newImage'])
    self._AssertContains('Service [{}] has been updated.'.format(svc_name), result,
                         'result')

  def testUpdate_Fail(self):
    svc_name = 'bar'
    command = '''kuberun clusters services update --cluster foo
    --cluster-location us-central1 {} --image newImage'''.format(svc_name)
    self.mock_bin_exec.return_value = bin_ops.BinaryBackedOperation.OperationResult(
  ...
  def testUpdateWithFlags(self, cmd_args, exe_args):
    svc_name = 'bar'
    command = '''kuberun clusters services update --cluster foo
    ...
    self.AssertExecuteCalledOnce(
        command_args=['clusters', 'services', 'update', '--cluster', 'foo',
                      '--cluster-location', 'us-central1', svc_name] + exe_args)
