...
class RevisionsListTest(test_base.PackageUnitTestBase):
  ...
  def testDescribe_Succeed(self):
    command = '''kuberun clusters domain-mappings describe --cluster foo
    ...
    self.AssertExecuteCalledOnce(
        command_args=['clusters', 'domain-mappings', 'describe', '--cluster',
  ...
  def testDescribe_Fail(self):
    ...
    --cluster-location us-central1 hello.example.com'''
    self.mock_bin_exec.return_value = bin_ops.BinaryBackedOperation.OperationResult(
        command, status=1, errors='no domain mapping hello.example.com found')
