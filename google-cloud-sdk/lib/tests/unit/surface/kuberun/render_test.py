...
class RenderTest(test_base.PackageUnitTestBase, parameterized.TestCase):
  ...
  def testRender(self):
    command = 'kuberun render'
    expected_out = 'Rendering 1 components in application'
    ...
    self.AssertExecuteCalledOnce(command_args=['render'])
    self._AssertContains(expected_out, result, 'result')

  def testRender_Fail(self):
    command = 'kuberun render'
    self.mock_bin_exec.return_value = (
        bin_ops.BinaryBackedOperation.OperationResult(
