...
"""ml-engine models add-iam-policy-binding tests."""
...
class AddIamPolicyBindingUnitTest(base.MlGaPlatformTestBase):
  ...
  def testAddIamPolicyBinding(self):
    response = self.Run('ml-engine models add-iam-policy-binding myModel '
                        '    --role roles/editor '
  ...
  def testAddIamPolicyBindingOutput(self):
    self.Run('ml-engine models add-iam-policy-binding myModel '
             '    --role roles/editor '
             '    --member user:test-user1@gmail.com')
  ...
  def testAddIamPolicyBindingRoleRequired(self):
    with self.AssertRaisesArgumentErrorMatches(
        'argument --role: Must be specified.'):
      self.Run('ml-engine models add-iam-policy-binding myModel '
               '    --member user:test-user1@gmail.com')

  def testAddIamPolicyBindingMemberRequired(self):
    with self.AssertRaisesArgumentErrorMatches(
        'argument --member: Must be specified.'):
      self.Run('ml-engine models add-iam-policy-binding myModel '
               '    --role roles/editor')

  def testAddIamPolicyBindingModelRequired(self):
    with self.AssertRaisesArgumentErrorMatches(
        'argument MODEL: Must be specified.'):
      self.Run('ml-engine models add-iam-policy-binding '
               '    --role roles/editor '
               '    --member user:test-user1@gmail.com')
