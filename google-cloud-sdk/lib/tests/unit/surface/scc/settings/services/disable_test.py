...
"""Tests for google3.third_party.py.tests.unit.surface.scc.settings.components.disable."""
...
class DisableTest(base.SecurityCenterSettingsUnitTestBase):

  def test_disable_service_organization(self):
    ...
    response = self.RunSccSettings('settings', 'services', 'disable',
                                   '--organization', '12345', '--service',
                                   'WEB_SECURITY_SCANNER')

    self.assertEqual(response, expected_response)

  def test_disable_service_folder(self):
  ...
  def test_disable_service_project(self):
  ...
  def test_disable_invalid_service(self):
    with self.AssertRaisesExceptionMatches(
        cli_test_base.MockArgumentError, 'argument --service: Invalid choice'):
      self.RunSccSettings('settings', 'services', 'disable', '--organization',
