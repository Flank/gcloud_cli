...
class AnalyzeEntitySentimentsTest(language_base.MlLanguageTestBase):
  """Test for the ml language analyze-entity-sentiment command."""

  def _ExpectAnalyzeEntitySentimentRequest(self, gcs_content_uri=None,
                                           content=None, encoding_type=None,
                                           content_type=None,
    ...
    """
    if entities:
      json_response = self._CreateEntitiesResponse(
          entities, response_language, with_sentiment=True)
    else:
      json_response = None
    annotate_request = self.messages.AnalyzeEntitySentimentRequest(
    ...
    if json_response:
      response = encoding.PyValueToMessage(
          self.messages.AnalyzeEntitySentimentResponse,
          json_response
      )
      self.client.documents.AnalyzeEntitySentiment.Expect(annotate_request,
                                                          response)
    if error:
      self.client.documents.AnalyzeEntitySentiment.Expect(annotate_request,
                                                          exception=error)
  ...
  def testBasicResult(self, track):
    """Test result returns as expected."""
    self.SetTrack(track)
    self._ExpectAnalyzeEntitySentimentRequest(
        content='puppies',
        entities=['puppies'],
        encoding_type=self.entity_sentiment_encoding_enum.UTF8,
        content_type=self.content_enum.PLAIN_TEXT)
    expected = self.messages.AnalyzeEntitySentimentResponse(
        ...
        language='eng')
    actual = self.Run('ml language analyze-entity-sentiment --content puppies')
  ...
  def testBasicOutput(self, track):
    ...
    self.Run('ml language analyze-entity-sentiment --content "puppies"')
  ...
  def testWithContentFile(self, track):
    ...
    self._ExpectAnalyzeEntitySentimentRequest(
        ...
        content_type=self.content_enum.PLAIN_TEXT)
    self.Run(
        'ml language analyze-entity-sentiment --content-file {}'.format(
            self.test_file))
  ...
  def testWithGCSContentFile(self, track):
    ...
    self._ExpectAnalyzeEntitySentimentRequest(
        ...
        encoding_type=self.entity_sentiment_encoding_enum.UTF8,
        content_type=self.content_enum.PLAIN_TEXT)
    self.Run('ml language analyze-entity-sentiment --content-file '
             'gs://bucket/file')
  ...
  def testWithEncodingType(self, track):
    ...
    self._ExpectAnalyzeEntitySentimentRequest(
        content='puppies',
        entities=['puppies'],
        encoding_type=self.entity_sentiment_encoding_enum.UTF16,
        content_type=self.content_enum.PLAIN_TEXT)
    self.Run('ml language analyze-entity-sentiment --content puppies '
             '--encoding-type UTF16')
  ...
  def testWithAllFlags(self, track):
    """Test extract entity sentiments command with all flags given."""
    self.SetTrack(track)
    self._ExpectAnalyzeEntitySentimentRequest(
        content='puppies',
        entities=['puppies'],
        encoding_type=self.entity_sentiment_encoding_enum.UTF32,
        content_type=self.content_enum.HTML,
        request_language='eng')
    self.Run('ml language analyze-entity-sentiment --content puppies '
             '--language eng --content-type HTML --encoding-type UTF32')

  def testWithNonGCSUrl(self, track):
    """Assert ContentFileError is raised for a non-GCS URL in --content-file."""
    self.SetTrack(track)
    non_gcs_url = 'https://bucket/file'
    with self.assertRaises(util.ContentFileError):
      self.Run('ml language analyze-entity-sentiment --content-file {}'.format(
          non_gcs_url))
  ...
  def testWithEmptyContent(self, track):
    ...
    with self.assertRaises(util.ContentError):
      self.Run('ml language analyze-entity-sentiment --content ""')
