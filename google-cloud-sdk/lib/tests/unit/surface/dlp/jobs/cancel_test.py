...
class CancelTest(base.DlpUnitTestBase):
  ...
  def testCancel(self, track):
    ...
    cancel_request = self.msg.DlpProjectsDlpJobsCancelRequest(
        googlePrivacyDlpV2CancelDlpJobRequest=self.msg.
        GooglePrivacyDlpV2CancelDlpJobRequest(),
        name=job_ref.RelativeName())
    self.client.projects_dlpJobs.Cancel.Expect(
        request=cancel_request, response=self.msg.GoogleProtobufEmpty())
    self.WriteInput('y')
    self.Run('dlp jobs cancel myjob')
