...
"""Tests for `gcloud tasks queues pause`."""
...
class QueuesPauseTest(test_base.CloudTasksTestBase):
  ...
  def testPause_RelativeName(self):
  ...
  def testPause_NonExistentQueue(self):
    self.queues_service.Pause.Expect(
        self.messages.CloudtasksProjectsLocationsQueuesPauseRequest(
            name='projects/{}/locations/us-central1/queues/{}'.format(
                self.Project(), 'my-queue')),
    ...
    with self.assertRaises(exceptions.HttpException):
      self.Run('tasks queues pause my-queue')

    self.AssertErrContains('Requested entity was not found.')
    self.AssertErrContains(constants.QUEUE_MANAGEMENT_WARNING)

  def testPause_Location(self):
    self.queues_service.Pause.Expect(
        self.messages.CloudtasksProjectsLocationsQueuesPauseRequest(
