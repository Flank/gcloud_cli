...
class AddIamPolicyBindingTest(test_base.CloudTasksTestBase):
  ...
  def testAddUserToQueue(self):
    member_string = 'user:test-user@gmail.com'
    expected_new_policy = self.messages.Policy(
        bindings=[self.messages.Binding(
            role=self.admin_role, members=[member_string])],
        etag=b'etag')
    self.queues_service.GetIamPolicy.Expect(
        ...
        response=self.messages.Policy(etag=b'etag'))
    self.queues_service.SetIamPolicy.Expect(
        self.messages.CloudtasksProjectsLocationsQueuesSetIamPolicyRequest(
            resource=self.queue_name,
            setIamPolicyRequest=self.messages.SetIamPolicyRequest(
                policy=expected_new_policy)),
        response=expected_new_policy)

    actual_new_policy = self.Run(
        'tasks queues add-iam-policy-binding {0} --member {1} '
        '--role {2}'.format(self.queue_name, member_string, self.admin_role))

    self.assertEqual(actual_new_policy, expected_new_policy)

  def testAddUserToQueue_Location(self):
    queue_id = 'my-queue'
    queue_name = (
    ...
    actual_new_policy = self.Run(
        'tasks queues add-iam-policy-binding {0} --member {1} '
        '--role {2} --location=us-central2'.format(
            queue_id, member_string, self.admin_role))

    self.assertEqual(actual_new_policy, expected_new_policy)


class RemoveIamPolicyBindingTest(test_base.CloudTasksTestBase):
  ...
  def testRemoveUserFromQueue(self):
    member_string = 'user:test-user@gmail.com'
    original_policy = self.messages.Policy(
        bindings=[self.messages.Binding(
            role=self.admin_role, members=[member_string])],
        etag=b'etag')
    expected_new_policy = self.messages.Policy(etag=b'etag')
    self.queues_service.GetIamPolicy.Expect(
        self.messages.CloudtasksProjectsLocationsQueuesGetIamPolicyRequest(
            resource=self.queue_name),
    ...
    actual_new_policy = self.Run(
        'tasks queues remove-iam-policy-binding {0} --member {1} '
        '--role {2}'.format(self.queue_name, member_string, self.admin_role))

    self.assertEqual(actual_new_policy, expected_new_policy)

  def testRemoveUserFromQueue_Location(self):
    queue_id = 'my-queue'
    queue_name = (
        'projects/fake-project/locations/us-central2/queues/{}'.format(
            queue_id))
