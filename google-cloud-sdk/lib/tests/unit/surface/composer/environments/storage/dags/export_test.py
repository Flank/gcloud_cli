...
class EnvironmentsStorageDagsExportGATest(base.GsutilShellingUnitTest,
  ...
  def testDagsExport(self, use_gsutil, isdir_mock, exec_mock):
    ...
    if use_gsutil:
      ...
      self.export_mock.assert_called_once_with(
          ...
          posixpath.join('dags', source), destination)
...
class EnvironmentsStorageDagsExportBetaTest(
    EnvironmentsStorageDagsExportGATest):

  def PreSetUp(self):
    self.SetTrack(calliope_base.ReleaseTrack.BETA)


class EnvironmentsStorageDagsExportAlphaTest(
    EnvironmentsStorageDagsExportBetaTest):
