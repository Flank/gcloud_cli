...
"""Unit tests for environments storage plugins export."""
...
class EnvironmentsStoragePluginsExportGATest(base.GsutilShellingUnitTest,
    ...
    if use_gsutil:
      ...
      fake_exec.AddCallback(
          ...
          self.MakeGsutilExecCallback(
              ['-m', 'cp', '-r',
               posixpath.join(self.test_gcs_bucket_path, 'plugins', source),
               destination]))
    else:
  ...
  @mock.patch('googlecloudsdk.core.execution_utils.Exec')
  @mock.patch('os.path.isdir', return_value=True)
  def testPluginsExportWildcardSource(self, use_gsutil, isdir_mock, exec_mock):
    ...
    self.RunEnvironments('storage', 'plugins', 'export',
                         '--project', self.TEST_PROJECT,
                         '--location', self.TEST_LOCATION,
                         '--environment', self.TEST_ENVIRONMENT_ID,
                         '--destination', destination)
  ...
  @mock.patch('googlecloudsdk.core.execution_utils.Exec')
  @mock.patch('os.path.isdir', return_value=True)
  def testPluginsExportWarning(self, source, isdir_mock, exec_mock):
    """Tests that when no SOURCE is provided, the entire folder is exported."""
    self.ExpectEnvironmentGet(
        self.TEST_PROJECT,
        self.TEST_LOCATION,
...
class EnvironmentsStoragePluginsExportBetaTest(
    EnvironmentsStoragePluginsExportGATest):

  def PreSetUp(self):
    self.SetTrack(calliope_base.ReleaseTrack.BETA)


class EnvironmentsStoragePluginsExportAlphaTest(
    EnvironmentsStoragePluginsExportBetaTest):
