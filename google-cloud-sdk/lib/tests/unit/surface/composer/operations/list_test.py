...
class OperationsListGATest(base.OperationsUnitTest):
  ...
  def testListFromPropertyFallthroughLocationMissing(self):
    with self.AssertRaisesExceptionRegexp(
        ...
        '--locations'):
      self.RunOperations('list', '--project', self.TEST_PROJECT)
  ...
  def _GenerateResponsesByLocation(self, location_ids):
    ...
    for location in location_ids:
      responses_by_location[location] = [
          self.messages.ListOperationsResponse(
              operations=[
...
class OperationsListBetaTest(OperationsListGATest):

  def PreSetUp(self):
    self.SetTrack(calliope_base.ReleaseTrack.BETA)


class OperationsListAlphaTest(OperationsListBetaTest):
