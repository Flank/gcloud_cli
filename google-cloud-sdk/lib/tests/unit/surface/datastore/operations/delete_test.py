...
class DeleteTest(base.DatastoreCommandUnitTest):
  ...
  def testDeleteRelativePath(self, track):
    ...
    self.mock_datastore_v1.projects_operations.Delete.Expect(
  ...
  def testDeleteAbsolutePath(self, track):
    ...
    self.RunDatastoreTest('operations delete {}'.format(operation_name_full))

  def _GetMockDeleteRequest(self, name):
    messages = operations.GetMessages()
    request = messages.DatastoreProjectsOperationsDeleteRequest()
    request.name = name
    return request
