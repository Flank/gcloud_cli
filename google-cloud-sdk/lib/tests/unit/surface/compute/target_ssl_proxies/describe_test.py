...
class TargetSslProxiesDescribeTest(test_base.BaseTest,
                                   test_case.WithOutputCapture):

  def testSimpleCase(self):
    messages = self.messages
    self.make_requests.side_effect = iter([
        [test_resources.TARGET_SSL_PROXIES_V1[0]],
    ])

    self.Run("""
        compute target-ssl-proxies describe target-ssl-proxy-1
        """)

    self.CheckRequests(
        [(self.compute.targetSslProxies,
          'Get',
          messages.ComputeTargetSslProxiesGetRequest(
              project='my-project',
