...
class InterconnectsPatchTest(test_base.BaseTest, parameterized.TestCase):
  ...
  @RunInTracks()
  def testPatchDescription(self, track, api_version):
    ...
    self.Run('compute interconnects patch my-interconnect '
             '--description "this is your interconnect"')

    self.CheckInterconnectRequest(description='this is your interconnect')

  @RunInTracks()
  def testPatchAdminEnabled(self, track, api_version):
    self._SetUp(track, api_version)
    messages = self.messages
    self.make_requests.side_effect = iter([
        [
    ...
    self.Run('compute interconnects patch my-interconnect ' '--admin-enabled')

    self.CheckInterconnectRequest(adminEnabled=True)

  @RunInTracks()
  def testPatchNoAdminEnabled(self, track, api_version):
    self._SetUp(track, api_version)
    messages = self.messages
    self.make_requests.side_effect = iter([
        [
  ...
  @RunInTracks()
  def testPatchRequestedLinkCount(self, track, api_version):
    self._SetUp(track, api_version)
    messages = self.messages
    self.make_requests.side_effect = iter([
        [
  ...
  @RunInTracks()
  def testPatchNocContactEmail(self, track, api_version):
    self._SetUp(track, api_version)
    messages = self.messages
    self.make_requests.side_effect = iter([
        [
  ...
  @RunInTracks()
  def testPatchMultipleFields(self, track, api_version):
    self._SetUp(track, api_version)
    messages = self.messages
    self.make_requests.side_effect = iter([
        [
  ...
  def testPatchLabels(self, track, api_version):
    self._SetUp(track, api_version)
