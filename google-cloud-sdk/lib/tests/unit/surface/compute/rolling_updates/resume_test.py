...
"""Tests for rolling-updates resume command."""
...
class UpdatesResumeTest(base.UpdaterMockTest):

  def testResume_Ok(self):
    self.mocked_client_v1beta1.rollingUpdates.Resume.Expect(
        messages.ReplicapoolupdaterRollingUpdatesResumeRequest(
    ...
    self.Run('alpha compute rolling-updates '
             '--zone=some-zone resume some-update')
    self.AssertErrContains(
        'Resumed [https://www.googleapis.com/replicapoolupdater/v1beta1'
        '/projects/{0}/zones/some-zone/rollingUpdates/some-update].'.format(
            self.Project()))

  def testResume_Failure_Errors(self):
    self.mocked_client_v1beta1.rollingUpdates.Resume.Expect(
        messages.ReplicapoolupdaterRollingUpdatesResumeRequest(
            project=self.Project(), zone=base.ZONE,
            rollingUpdate='some-update'),
    ...
    with self.AssertRaisesToolExceptionRegexp(
        r'could not resume \[https://www.googleapis.com/'
        r'replicapoolupdater/v1beta1/projects/{0}/zones/'
        r'some-zone/rollingUpdates/some-update\]'.format(self.Project())):
      self.Run('alpha compute rolling-updates '
               '--zone=some-zone resume some-update')

  def testResume_Failure_MissingArgument_Zone(self):
    try:
      self.Run('alpha compute rolling-updates resume some-update')
