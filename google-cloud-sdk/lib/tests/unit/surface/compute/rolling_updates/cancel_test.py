...
"""Tests for rolling-updates cancel command."""
...
class UpdatesCancelTest(base.UpdaterMockTest):

  def testCancel_Ok(self):
    self.mocked_client_v1beta1.rollingUpdates.Cancel.Expect(
        messages.ReplicapoolupdaterRollingUpdatesCancelRequest(
    ...
    self.Run('alpha compute rolling-updates '
             '--zone=some-zone cancel some-update')
    self.AssertErrContains(
        'Cancelled [https://www.googleapis.com/replicapoolupdater/v1beta1'
        '/projects/{0}/zones/some-zone/rollingUpdates/some-update].'.format(
            self.Project()))

  def testCancel_Failure_Errors(self):
    self.mocked_client_v1beta1.rollingUpdates.Cancel.Expect(
        messages.ReplicapoolupdaterRollingUpdatesCancelRequest(
            project=self.Project(), zone=base.ZONE,
            rollingUpdate='some-update'),
    ...
    with self.AssertRaisesToolExceptionRegexp(
        r'could not cancel \[https://www.googleapis.com/replicapoolupdater/'
        r'v1beta1/projects/{0}/zones/some-zone/'
        r'rollingUpdates/some-update\]'.format(self.Project())):
      self.Run('alpha compute rolling-updates '
               '--zone=some-zone cancel some-update')

  def testCancel_Failure_MissingArgument_Zone(self):
    try:
      self.Run('alpha compute rolling-updates cancel some-update')
