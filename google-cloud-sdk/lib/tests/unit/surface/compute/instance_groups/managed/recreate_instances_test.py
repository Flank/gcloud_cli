...
"""Tests for the instance-groups managed recreate-instances subcommand."""
...
class InstanceGroupManagersRecreateInstancesTest(test_base.BaseTest):
  ...
  def testWithTwoInstances(self):
    ...
    instances = self.messages.InstanceGroupManagersRecreateInstancesRequest(
    ...
    self.CheckRequests(
        [(self.compute.instanceGroupManagers, 'RecreateInstances',
          recreate_request)],
    )
  ...
  def testWithUri(self):
    ...
    instance_uri = ('{0}/projects/my-project/zones/central2-a/instances/'
                    'instance1'.format(self.compute_uri))
    self.Run('compute instance-groups managed recreate-instances {0} --zone '
             'central2-a --instances {1}'.format(igm_uri, instance_uri))
  ...
  def testZonePrompt(self):
    ...
    self.WriteInput('5\n')
    self.Run('compute instance-groups managed recreate-instances group-1 '
             '--instances instance1')
...
class InstanceGroupManagersRecreateInstancesRegionalTest(test_base.BaseTest):
  ...
  def testWithUri(self):
    ...
    self.Run("""
        compute instance-groups managed recreate-instances {0}
            --region central2
            --instances {1}
