...
class WithIAPApiTest(test_base.AlphaUpdateTestBase):
  ...
  def testWithIAPDisabled(self):
    self.RunUpdate('backend-service-1 --iap disabled')
    self.CheckResults(self.messages.BackendServiceIAP(enabled=False))
    self.AssertErrEquals('')

  def testWithIAPEnabled(self):
    self.RunUpdate('backend-service-1 --iap enabled')
    self.CheckResults(self.messages.BackendServiceIAP(enabled=True))

  def testWithIAPEnabledAndNonHttpsProtocol(self):
    self.RunUpdate('backend-service-1 --iap enabled --protocol=HTTP')
    self.CheckResults(self.messages.BackendServiceIAP(enabled=True))
    self.AssertErrEquals(self._lb_warning + self._non_https_warning)

  def testWithIAPEnabledAndHttpsProtocol(self):
    self.RunUpdate('backend-service-1 --iap enabled --protocol=HTTPS')
    self.CheckResultsWithProtocol(
  ...
  def testWithIAPDisabledAndNonHttpsProtocol(self):
    self.RunUpdate('backend-service-1 --iap disabled --protocol=HTTP')
    self.CheckResults(self.messages.BackendServiceIAP(enabled=False))
    self.AssertErrEquals('')

  def testWithIAPDisabledAndHttpsProtocol(self):
    self.RunUpdate('backend-service-1 --iap disabled --protocol=HTTPS')
    self.CheckResultsWithProtocol(
  ...
  def testWithIAPEnabledWithCredentials(self):
    ...
    self.CheckResults(
        self.messages.BackendServiceIAP(
            enabled=True,
  ...
  def testWithIAPCredentialsOnly(self):
    ...
    try:
      self._backend_services[0].iap = self.messages.BackendServiceIAP(
          enabled=True, oauth2ClientId='ID', oauth2ClientSecret='SECRET')
  ...
  def testInvalidIAPEmpty(self):
    self.assertRaisesRegex(
        ...
        r'--iap$', self.RunUpdate, 'backend-service-1 --iap=""')

  def testInvalidIapArgCombinationEnabledDisabled(self):
    self.assertRaisesRegex(
        exceptions.InvalidArgumentException,
        '^Invalid value for \\[--iap\\]: Must specify only one '
        'of \\[enabled\\] or \\[disabled\\]$', self.RunUpdate,
        'backend-service-1 --iap enabled,disabled')
