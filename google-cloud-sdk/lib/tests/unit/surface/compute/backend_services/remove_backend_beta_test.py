...
class BackendServicesRemoveBackendTest(test_base.BaseTest):
  ...
  def testUriSupport(self):
    ...
    self.Run("""
        compute backend-services remove-backend
          ...
          --instance-group https://www.googleapis.com/compute/beta/projects/my-project/zones/us-central1-a/instanceGroups/my-group-1
          --global
  ...
  def testWithExistingBackendZonalIG(self):
    messages = self.messages
    self.make_requests.side_effect = [
        [messages.BackendService(
            name='my-backend-service',
