...
class HealthChecksUpdateHttpsAlphaTest(HealthChecksUpdateHttpsBetaTest):
  ...
  def testLogConfig(self, enable_logs_flag, enable_logs):
    ...
    self.Run("""
        compute health-checks update https my-health-check {0}""".format(
            enable_logs_flag) + self.global_flag())
  ...
  def testEnableToDisableLogConfig(self):
    ...
    self.Run(
        """compute health-checks update https my-health-check --no-enable-logging"""
        + self.global_flag())
