...
class OrgSecurityPoliciesCopyRulesAlphaTest(test_base.BaseTest):
  ...
  def CheckOrgSecurityPolicyRequest(self, **kwargs):
    self.CheckRequests([
        (self.compute.organizationSecurityPolicies, 'CopyRules',
         self.messages.ComputeOrganizationSecurityPoliciesCopyRulesRequest(
             securityPolicy='9999999', sourceSecurityPolicy='12345'))
    ])

  def testCopyRulesOrgSecurityPolicy(self):
    self.Run('compute org-security-policies copy-rules 9999999 '
             '--source-security-policy 12345')
