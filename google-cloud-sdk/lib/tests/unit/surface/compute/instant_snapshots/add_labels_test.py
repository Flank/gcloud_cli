...
"""Tests for instant snapshot add-labels subcommand."""
...
from tests.lib.surface.compute import instant_snapshots_labels_test_base


class AddLabelsTest(
    instant_snapshots_labels_test_base.InstantSnapshotsLabelsTestBase):
  """Instant snapshot add-labels test."""
  ...
  def testZonalUpdateValidInstantSnapshotsWithLabelsAndRemoveLabels(self):
    ips_ref = self._GetInstantSnapshotRef('ips-1', zone='zone-1')
    ...
    ips = self._MakeInstantSnapshotProto(
        labels=ips_labels, fingerprint=b'fingerprint-42')
    updated_ips = self._MakeInstantSnapshotProto(labels=edited_labels)
    ...
    response = self.Run(
        'compute instant-snapshots add-labels {} --labels {}'.format(
            ips_ref.SelfLink(), ','.join(
                ['{0}={1}'.format(pair[0], pair[1]) for pair in add_labels])))
    self.assertEqual(response, updated_ips)

  def testRegionalUpdateValidInstantSnapshotsWithLabelsAndRemoveLabels(self):
    ips_ref = self._GetInstantSnapshotRef('ips-1', region='region-1')
  ...
  def testNoLabelsSpecified(self):
    with self.AssertRaisesArgumentErrorMatches(
      ...
      self.Run('compute instant-snapshots add-labels label-1')

  def testInvalidLabel(self):
    ips_ref = self._GetInstantSnapshotRef('ips-1', zone='atlanta')
    ...
    with self.AssertRaisesHttpExceptionMatches('Invalid label: +notvalid'):
      self.Run('compute instant-snapshots add-labels {} --labels {} '.format(
          ips_ref.SelfLink(),
