...
class CreateTestAlpha(unit_test_base.SCMUnitTestBase):
  """Unit tests for endpoints quota create command."""
  ...
  def testCreate(self):
    ...
    self.Run('endpoints quota create --service=example.googleapis.com '
...
""",
  ...
  def testCreate_force(self):
    ...
    self.Run('endpoints quota create --service=example.googleapis.com '
             '--consumer=projects/helloworld '
             '--metric=example.googleapis.com/mutate_requests '
             '--unit=1/min/{project} --value=666 --force')
    self.AssertErrEquals(
        """\
Operation "operations/123" finished successfully.
""",
