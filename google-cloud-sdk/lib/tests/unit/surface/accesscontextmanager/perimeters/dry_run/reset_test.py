...
"""Tests for `gcloud access-context-manager perimeters dry-run reset`."""
...
class DryRunResetTestBeta(accesscontextmanager.Base):
  ...
  def testReset(self):
    ...
    perimeter_update = self.messages.ServicePerimeter(
        spec=None, useExplicitDryRunSpec=False)
    ...
    result = self.Run(
        'access-context-manager perimeters dry-run reset MY_PERIMETER '
        '   --policy 123')

    self.assertEqual(result, perimeter_after)


class DryRunResetTestAlpha(DryRunResetTestBeta):
