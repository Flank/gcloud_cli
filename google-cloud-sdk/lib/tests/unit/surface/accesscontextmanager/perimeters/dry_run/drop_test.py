...
class DryRunDropTestBeta(accesscontextmanager.Base):
  ...
  def testDrop(self):
    ...
    result = self.Run(
        'access-context-manager perimeters dry-run drop MY_PERIMETER '
        '   --policy 123')

    self.assertEqual(result, perimeter_after)


class DryRunDropTestAlpha(DryRunDropTestBeta):
