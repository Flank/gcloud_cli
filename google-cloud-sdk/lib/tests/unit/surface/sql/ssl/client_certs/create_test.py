...
class _BaseClientCertsCreateTest(object):
  ...
  def testClientCertsCreate(self):
    ...
    self.Run('sql ssl client-certs create --instance=integration-test newcert '
             '{0}'.format(cert_file))
...
""", normalize_space=True)
  ...
  def testClientCertsCreateBadDestination(self):
    with self.assertRaisesRegex(
        exceptions.ArgumentError,
        r'unable to write \[/foobar\]'):
      self.Run('sql ssl client-certs create newcert /foobar '
               '--instance=integration-test')

  def testClientCertsCreateAlreadyExists(self):
