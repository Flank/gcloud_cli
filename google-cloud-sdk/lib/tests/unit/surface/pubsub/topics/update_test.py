...
class TopicsUpdateTest(base.CloudPubsubTestBase):
  ...
  def testUpdateMessageStoragePolicyAllowedPersistenceRegions(
      self):
    topic_ref = util.ParseTopic('topic', self.Project())
    update_req = self.msgs.PubsubProjectsTopicsPatchRequest(
        updateTopicRequest=self.msgs.UpdateTopicRequest(
            topic=self.msgs.Topic(
                name=topic_ref.RelativeName(),
                messageStoragePolicy=self.msgs.MessageStoragePolicy(
                    allowedPersistenceRegions=['us-central1', 'us-central2'])),
            updateMask=('messageStoragePolicy')),
        name=topic_ref.RelativeName())
    self.svc.Expect(request=update_req, response=self.msgs.Topic())  # Ignore
    self.Run(
        'pubsub topics update topic --message-storage-policy-allowed-regions=us-central1,us-central2'
    )
    self.AssertErrEquals('Updated topic [{0}].\n'.format(
...
class TopicsUpdateTestBeta(TopicsUpdateTest):
