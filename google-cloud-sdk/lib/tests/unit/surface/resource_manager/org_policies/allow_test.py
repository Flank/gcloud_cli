...
class OrgPoliciesAllowTest(testbase.OrgPoliciesUnitTestBase):
  ...
  def testAllowOrgPolicyRestoreDefault(self):
    self.mock_projects.GetOrgPolicy.Expect(
        self.ExpectedGetRequest(self.PROJECT_ARG, self.WHITELIST_CONSTRAINT),
        self.RestoreDefaultPolicy(self.WHITELIST_CONSTRAINT))
    self.mock_projects.SetOrgPolicy.Expect(
        self.ExpectedSetRequest(
            self.PROJECT_ARG, self.WhitelistPolicy([self.VALUE_A,
                                                    self.VALUE_B])),
        self.WhitelistPolicy([self.VALUE_A, self.VALUE_B]))
    self.assertEqual(
        self.DoRequest(self.PROJECT_ARG),
...
class OrgPoliciesAllowAlphaTest(OrgPoliciesAllowTest):

  def PreSetUp(self):
    self.track = calliope_base.ReleaseTrack.ALPHA


class OrgPoliciesAllowBetaTest(OrgPoliciesAllowTest):
