...
"""Tests for google3.third_party.py.tests.unit.surface.ai.hp_tuning_jobs.cancel."""
...
class CancelHptuningJobUnitTestAlpha(cli_test_base.CliTestBase,
  ...
  def testCancelHptuningJobAlpha(self):
    ...
    self.mock_client.projects_locations_hyperparameterTuningJobs.Cancel.Expect(
        request, response=self.messages.GoogleProtobufEmpty())
    self.RunCommand('cancel', '1', '--region={}'.format(self.region))
    self.AssertErrContains(
        'Using endpoint [https://us-central1-aiplatform.googleapis.com/')
    self.AssertErrContains(
        'Request to cancel hyperparameter tuning job [1] has been sent')
    self.AssertErrContains('gcloud alpha ai hp-tuning-jobs describe 1')


class CancelHptuningJobUnitTestBeta(CancelHptuningJobUnitTestAlpha):
