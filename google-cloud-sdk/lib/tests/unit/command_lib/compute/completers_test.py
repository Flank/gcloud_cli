...
_COMMAND_RESOURCES = {
    'compute.instances.list':
        completer_test_data.INSTANCE_URIS,
    'compute.health-checks.list':
        _Uris(health_check_resources.HEALTH_CHECKS),
    'compute.http-health-checks.list':
        _Uris(health_check_resources.HTTP_HEALTH_CHECKS),
    'compute.https-health-checks.list':
        _Uris(health_check_resources.HTTPS_HEALTH_CHECKS_V1),
...
class ComputeCompleterTest(completer_test_base.CompleterBase):
  ...
  def testRegionsCompleter(self):
    ...
    self.assertCountEqual([
        'asia-east1', 'asia-northeast1', 'europe-west1', 'us-central1',
        'us-central2', 'us-east1', 'us-west1'
    ], completer.Complete('', self.parameter_info))
    self.assertCountEqual(['europe-west1'],
                          completer.Complete('e', self.parameter_info))
    self.assertCountEqual(['europe-west1', 'us-west1'],
                          completer.Complete('*w*', self.parameter_info))
    self.assertCountEqual([], completer.Complete('q', self.parameter_info))

  def testZonesCompleter(self):
    completer = self.Completer(completers.ZonesCompleter)
    self.assertCountEqual([
        'asia-east1-a', 'asia-east1-b', 'asia-east1-c', 'asia-northeast1-a',
        'asia-northeast1-b', 'asia-northeast1-c', 'europe-west1-b',
        'europe-west1-c', 'europe-west1-d', 'us-central1-a', 'us-central1-b',
        'us-central1-c', 'us-central1-f', 'us-central2-a', 'us-east1-b',
    ...
    self.assertCountEqual(
        ...
        completer.Complete('e', self.parameter_info))
    self.assertCountEqual([
        'europe-west1-b', 'europe-west1-c', 'europe-west1-d', 'us-west1-a',
        'us-west1-b'
    ], completer.Complete('*w*', self.parameter_info))
    self.assertCountEqual([], completer.Complete('q', self.parameter_info))
...
class ComputeFlagCompleterTest(completer_test_base.FlagCompleterBase):
  ...
  def testInstancesCompleter(self):
    ...
    self.assertEqual(228, len(completer.Complete('', self.parameter_info)))
    self.assertCountEqual([
  ...
  def testInstancesCompleterWithArgs(self):
    completer = self.Completer(
        ...
        command_resources=_COMMAND_RESOURCES)
    self.assertCountEqual(['my_b_instance'],
                          completer.Complete('my_b', self.parameter_info))
    self.assertCountEqual(['my_a_instance'],
...
class ComputeGRICompleterTest(completer_test_base.GRICompleterBase):
  ...
  def testHealthChecksCompleter(self):
    completer = self.Completer(
        completers.HealthChecksCompleter, command_resources=_COMMAND_RESOURCES)
    self.assertEqual(7, len(completer.Complete('', self.parameter_info)))
    self.assertCountEqual([
        'health-check-grpc:my-project',
    ...
    self.assertCountEqual(['health-check-http-2:my-project'],
  ...
  def testHttpHealthChecksCompleter(self):
    completer = self.Completer(
        ...
        command_resources=_COMMAND_RESOURCES)
    self.assertEqual(2, len(completer.Complete('', self.parameter_info)))
    self.assertCountEqual([
        'health-check-1:my-project',
        'health-check-2:my-project',
    ], completer.Complete('', self.parameter_info))
    self.assertCountEqual(['health-check-2:my-project'],
                          completer.Complete('*-2', self.parameter_info))
  ...
  def testHttpsHealthChecksCompleter(self):
    ...
    self.assertCountEqual(['https-health-check-2:my-project'],
                          completer.Complete('*-2', self.parameter_info))
  ...
  def testInstanceTemplatesCompleter(self):
    completer = self.Completer(
        ...
        command_resources=_COMMAND_RESOURCES)
    self.assertEqual(3, len(completer.Complete('', self.parameter_info)))
    self.assertCountEqual([
        'instance-template-1:my-project',
        'instance-template-2:my-project',
        'instance-template-3:my-project',
    ], completer.Complete('', self.parameter_info))
    self.assertCountEqual(['instance-template-2:my-project'],
                          completer.Complete('*-2', self.parameter_info))

  def testInstancesCompleter(self):
    completer = self.Completer(
        completers.InstancesCompleter, command_resources=_COMMAND_RESOURCES)
    self.assertEqual(228, len(completer.Complete('', self.parameter_info)))
    self.assertCountEqual([
  ...
  def testInstancesCompleterWithArgs(self):
    ...
    self.assertCountEqual(['my_a_instance'],
                          completer.Complete('my_a', self.parameter_info))
    self.assertCountEqual(
        ['my_a_instance:europe-west1-d', 'my_a_instance:us-east1-d'],
        completer.Complete('my_a:*d', self.parameter_info))
    self.assertCountEqual(['my_a_instance:us-east1-b:your_z_project'],
                          completer.Complete('my_a:*:your_z',
                                             self.parameter_info))
    self.assertCountEqual([],
                          completer.Complete('my_a:*:my_y',
                                             self.parameter_info))
  ...
  def testInstancesCompleterWithArgs2(self):
    completer = self.Completer(
        completers.InstancesCompleter,
        args={
            'project': 'your_z_project',
            'zone': 'us-east1-b'
        },
        command_resources=_COMMAND_RESOURCES)
    self.assertCountEqual([
        'my_b_instance:asia-east1-b', 'my_b_instance:asia-northeast1-b',
        'my_b_instance:europe-west1-b', 'my_b_instance:us-central1-b',
        'my_b_instance', 'my_b_instance:us-west1-b'
    ], completer.Complete('my_b:*b:*_z_*', self.parameter_info))
    self.assertCountEqual(['my_a_instance'],
                          completer.Complete('my_a', self.parameter_info))
    self.assertCountEqual(
  ...
  def testMachineImagesCompleter(self):
    completer = self.Completer(
        completers.MachineImagesCompleter, command_resources=_COMMAND_RESOURCES)
    self.assertEqual(2, len(completer.Complete('', self.parameter_info)))
    self.assertCountEqual([
        'machine-image-1:my-project',
        'machine-image-2:my-project',
    ], completer.Complete('', self.parameter_info))
    self.assertCountEqual(['machine-image-2:my-project'],
                          completer.Complete('*-2', self.parameter_info))

  def testSearchHealthChecksCompleter(self):
    completer = self.Completer(
        completers.SearchHealthChecksCompleter,
        search_resources=_SEARCH_RESOURCES)
    self.assertEqual(7, len(completer.Complete('', self.parameter_info)))
    self.assertCountEqual([
        'health-check-grpc:my-project',
  ...
  def testSearchHttpHealthChecksCompleter(self):
    completer = self.Completer(
        completers.SearchHttpHealthChecksCompleter,
        search_resources=_SEARCH_RESOURCES)
    self.assertEqual(2, len(completer.Complete('', self.parameter_info)))
    self.assertCountEqual([
        'health-check-1:my-project',
  ...
  def testSearchHttpsHealthChecksCompleter(self):
    completer = self.Completer(
        completers.SearchHttpsHealthChecksCompleter,
        search_resources=_SEARCH_RESOURCES)
    self.assertEqual(2, len(completer.Complete('', self.parameter_info)))
    self.assertCountEqual([
        'https-health-check-1:my-project',
  ...
  def testSearchInstanceTemplatesCompleter(self):
    completer = self.Completer(
        completers.SearchInstanceTemplatesCompleter,
        search_resources=_SEARCH_RESOURCES)
    self.assertEqual(3, len(completer.Complete('', self.parameter_info)))
    self.assertCountEqual([
        'instance-template-1:my-project',
  ...
  def testSearchInstancesCompleter(self):
    completer = self.Completer(
        completers.SearchInstancesCompleter, search_resources=_SEARCH_RESOURCES)
    self.assertEqual(228, len(completer.Complete('', self.parameter_info)))
    self.assertCountEqual([
        'my_b_instance:asia-east1-b:your_z_project',
        'my_b_instance:asia-northeast1-b:your_z_project',
