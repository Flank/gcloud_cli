...
def CheckFailResult(transport,
  ...
  def CreateFailure(url, err):
    msg = 'Cannot reach {0} with {1} ({2})'.format(url, transport,
                                                   type(err).__name__)
...
class ReachabilityCheckerTests(diagnostics_test_base.DiagnosticTestBase):
  ...
  def SetUp(self):
    self.requests_mock = self.StartObjectPatch(
        core_requests, 'GetSession').return_value.request
    self.requests_mock.return_value = 'dummy_response'
    self.http_request_mock = self.StartObjectPatch(http,
                                                   'Http').return_value.request
  ...
  def testCantConnectNoInternet(self):
    ...
    expected_result = CheckFailResult('httplib2',
  ...
  def testCantConnectNoInternetTimeout(self):
    ...
    expected_result = CheckFailResult(
        'httplib2', ['https://www.googleapis.com'], exception=socket.timeout())
    actual_result, actual_fixer = self.reachability_checker.Check(
        ['https://www.googleapis.com'])
    self.AssertResultEqual(expected_result, actual_result)
    self.assertEqual(CHANGE_PROXY, actual_fixer)
  ...
  def testCantConnectSSLError(self):
    ...
    expected_result = CheckFailResult(
        'httplib2', ['https://www.googleapis.com'], exception=ssl.SSLError())
    actual_result, actual_fixer = self.reachability_checker.Check(
        ['https://www.googleapis.com'])
    self.AssertResultEqual(expected_result, actual_result)
    self.assertEqual(CHANGE_PROXY, actual_fixer)
  ...
  def testCantConnectSocks5ProxyError(self):
    ...
    expected_result = CheckFailResult(
        'httplib2', ['https://accounts.google.com'],
        exception=socks.HTTPError((403, 'Forbidden')))
    actual_result, actual_fixer = self.reachability_checker.Check(
  ...
  def testEmptyUrlList(self):
    self.http_request_mock.return_value = 'dummy_response', 'dummy_content'
    expected_result = check_base.Result(
        passed=True, message='No URLs to check.')
    actual_result, actual_fixer = self.reachability_checker.Check([])
    self.AssertResultEqual(expected_result, actual_result)
    self.assertIsNone(actual_fixer)


class ReachabilityCheckerRequestsTests(
    diagnostics_test_base.DiagnosticTestBase):
  ...
  def testRequestsCantConnectNoInternet(self):
    self.http_request_mock.return_value = 'dummy_response', 'dummy_content'
    self.requests_mock.side_effect = requests.exceptions.RequestException
    expected_result = CheckFailResult(
        'requests', ['https://www.googleapis.com'],
...
class NetworkDiagnosticTests(diagnostics_test_base.DiagnosticTestBase):
  ...
  _FAIL_RESULT = CheckFailResult('requests', network_diagnostics.DefaultUrls())
  ...
  def testNoNetworkIssues(self):
    ...
    self.AssertErrContains('Network diagnostic passed (1/1 checks passed).')
  ...
  def testNetworkIssuesProxyChangesMadeIssuesFixed(self):
    self.run_check_mock.side_effect = [(self._FAIL_RESULT, self.fixer_mock),
...
class DefaultUrlsTests(test_case.Base):
  ...
  def SetUp(self):
    ...
    self.StartObjectPatch(
        config.INSTALLATION_CONFIG, 'snapshot_url', return_value='')

  def testNoSnapshotUrls(self):
    self.assertEqual(self._BASE_DEFAULT_URLS, network_diagnostics.DefaultUrls())
  ...
  def testSnapshotUrlPropertySetNonlocalMultipleUrls(self):
    ...
    self.assertEqual(
        self._BASE_DEFAULT_URLS + ['https://repo1.repo', 'https://repo2.repo'],
        network_diagnostics.DefaultUrls())
