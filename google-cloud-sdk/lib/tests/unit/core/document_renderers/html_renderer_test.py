...
class HTMLMarkdownTests(test_base.Markdown):
  ...
  def testHTMLSynopsis(self):
    markdown = textwrap.dedent("""\
        ...
        `gcloud compute backend-services update-backend` _NAME_ *--instance-group*=_INSTANCE_GROUP_ [*--no-launch-browser*] [*--balancing-mode*=_BALANCING_MODE_] [*--capacity-scaler*=_CAPACITY_SCALER_] [*--description*=_DESCRIPTION_] [*--max-utilization*=_MAX_UTILIZATION_] [*--instance-group-zone*=_INSTANCE_GROUP_ZONE_ | *--zone*=_ZONE_] [*--max-connections*=_MAX_CONNECTIONS_ | *--max-connections-per-instance*=_MAX_CONNECTIONS_PER_INSTANCE_ | *--max-rate*=_MAX_RATE_ | *--max-rate-per-instance*=_MAX_RATE_PER_INSTANCE_] [_GCLOUD_WIDE_FLAG ..._]
