...
class SetupTest(e2e_base.WithServiceAuth, e2e_base.WithServiceAccountFile):
  ...
  def TearDown(self):
    with e2e_base.RefreshTokenAuth() as _:
      if os.path.exists(_LOCAL_CREDENTIAL_FILE_PATH):
        with open(_LOCAL_CREDENTIAL_FILE_PATH) as f:
          credential_json = json.load(f)
          key_id = credential_json['private_key_id']

        self.Run(('iam service-accounts keys delete {0} '
                  '--iam-account={1}').format(key_id, self.local_account_email))
  ...
  def testCreateServiceAccountCredential(self):
    ...
    with refresh_token as auth, local_credential_variable as _:
      command = ('local setup --project {0} --kubernetes-file={1} '
    ...
    pod_specs = [spec for spec in pods_and_services if spec['kind'] == 'Pod']
    for spec in pod_specs:
      env_vars = yaml_helper.GetAll(spec, path=('spec', 'containers', 'env'))
      credential_vars = (
          var['value']
          for var in env_vars
