...
class CreateTest(base.DebugIntegrationTestWithTargetArg):
  ...
  def testSnapshotList(self):
    self.RunDebug(['snapshots', 'create', 'dummy_file:45'])
    self.AssertOutputContains('id: ', normalize_space=True)
    self.AssertOutputContains('location: dummy_file:45', normalize_space=True)
    self.AssertOutputContains('status: ACTIVE', normalize_space=True)
    self.AssertOutputContains('consoleViewUrl: https', normalize_space=True)
