...
"""Tests for `datastore operations` commands."""

from tests.lib import test_case
from tests.lib.surface.datastore import e2e_base


class OperationsIntegrationTest(e2e_base.DatastoreE2ETestBase):
  ...
  def testOperationsList(self):
    self.Run('datastore operations list --page-size=10 --limit=20')

  def testOperationsDescribe(self):
    operations = list(self.Run('datastore operations list'))
    if not operations:
      self.skipTest('No operations to describe')
    operation_id = operations[0].name
    operation = self.Run(
        'datastore operations describe {0}'.format(operation_id))
