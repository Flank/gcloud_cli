...
class SimulateMaintenanceEventTest(e2e_instances_test_base.InstancesTestBase):
  ...
  def testSimulateMaintenanceEvent(self):
    ...
    except core_exceptions.MultiError as err:
      # The SimulateMaintenanceEvent API can transiently fail with this
      # message under normal circumstances.  This is not a CLI error, so
      # pass the test; if this is happening too often service tests at lower
      # levels will catch it.
      if 'temporary concurrency limitations' not in '{}'.format(err):
        raise err
